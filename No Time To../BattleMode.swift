...
class BattleScene: SKScene {
    ...
    override func didMoveToView(view: SKView) {
        ...
        //let boss1: Boss1 = Boss1(imageNamed: "creatureCircle2.png")
        //self.createSprite(boss1, andWithPosition: CGPointMake(self.frame.size.width/2, self.frame.size.height/2), andWithZPosition: 2, andWithString: "boss1", andWithSize: 4, andWithParent: self)
        
        // let boss4: SKSpriteNode = SKSpriteNode(imageNamed: "creature2.png")
        // self.createSprite(boss4, andWithPosition: CGPointMake(self.frame.size.width/2, self.frame.size.height/2), andWithZPosition: 2, andWithString: "boss1", andWithSize: 4, andWithParent: self)
        // var nodeA: transformTexture = transformTexture(imageNamed: "creature2.png")
        
        //boss4.texture = nodeA.texture
        
        
        
        /*self.runAction(SKAction.repeatAction(SKAction.sequence([
         SKAction.waitForDuration(0.25), SKAction.runBlock({nodeA.shatterPrime()}), SKAction.runBlock({boss4.texture = nodeA.texture}),
         ]), count: 11))
         */
    ...
    func createRealBoss(){
        if(currentLevel == 1){
            ...
            self.createSprite(boss2, andWithPosition: CGPointMake(self.frame.size.width, self.frame.size.height/2), andWithZPosition: 2, andWithString: "boss1", andWithSize: 4, andWithParent: self)
            self.boss2.canIPlzHaveAMomentToPutMymakeupOn = true
        }
    }
        ...
        if(wasKilledByADemonWhoToreItselfFreeFromAHumanSoul == false && swipeBoolDone == true){
            ...
            if(currentLevel == 1){
                ...
                if(currentRound == 4 && boss2.name != "dud" && player.Health > 0 && boss2.name != "did"){
                    print("st11")
                    if(boss2.progressFraction >= 1.0){
                        print("Strike")
                        boss2.strikeAt(player.position)
                        player.doDamage(boss2.Strength)
                        boss2.progressFraction = 0.0
                    }
                }
                if(boss2.name == "dud"){
                    boss2.name = "did"
    ...
    func createEnemy() {
        ...
        if(currentRound == 1){
            ...
            //midEnemy.type = SCARECROW
            midEnemy.begin(SCARECROW, andWithStartValue: 0.0)
            ...
            //self.createSprite(DOD, andWithPosition: CGPointMake(self.frame.size.width/2, self.frame.size.height/2 + 200), andWithZPosition: 2, andWithString: "dod", andWithSize: 5, andWithParent: self)
            
            DOD.size = CGSizeMake(70 * 5, 7 * 5)
            ...
            if(currentLevel == 1){
                ...
                highEnemy.begin(SHADOW, andWithStartValue: 0.0)
                lowEnemy.begin(SHADOW, andWithStartValue: 0.25)
            }
            ...
            midEnemy.begin(GOODSLIME, andWithStartValue: 0.0)
    ...
    func attackEnemy(whichEnemy: BaseEnemy) {
        ...
        }else if(currentRound == 4 && boss1.canIPlzHaveAMomentToPutMymakeupOn == true){
